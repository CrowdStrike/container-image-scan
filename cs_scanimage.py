...
def main():
    try:
        ...
        if  f_vuln_score >= int(score):
             log.error("Exiting: Vulnerability score threshold exceeded: '%s' out of '%s'", f_vuln_score, score)
             sys.exit(ScanStatusCode.Vulnerability.value)
        else:
             log.info("Vulnerability score threshold not met: '%s' out of '%s'", f_vuln_score, score)
