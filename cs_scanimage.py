        ...
        if vulnerabilities is not None:
            for vulnerability in vulnerabilities:
                ...
                if severity.lower() == 'low':
                    vuln_score = vuln_score + ScanStatusCode.LowScore.value
                if severity.lower() == 'medium':
                    vuln_score = vuln_score + ScanStatusCode.MediumScore.value
                if severity.lower() == 'high':
                    vuln_score = vuln_score + ScanStatusCode.HighScore.value
                if severity.lower() == 'critical':
                    vuln_score = vuln_score + ScanStatusCode.CriticalScore.value
        return vuln_score
...
def main():
    try:
        ...
        vuln_score = scan_report.get_alerts_vuln()

        if int(vuln_score) >= int(score):
             log.info("Vulnerability score threshold exceeded: '%s' out of '%s'", scan_report.status_code(), score)
             sys.exit(ScanStatusCode.HighVulnerability.value)
        else:
             log.info("Vulnerability score threshold not met: '%s' out of '%s'", scan_report.status_code(), score)
